...
import { Send, PlusCircle, Hash, X, Info } from 'lucide-react';
...
export default function Chat() {
  ...
  const handleSend = async (e?: React.FormEvent) => {
    ...
    try {
      ...
      // Optimistically add if socket doesn't echo back
      setMessages(prev => {
  ...
  return (
    <div className="h-[calc(100vh-10rem)] flex overflow-hidden border border-outline-variant/30 rounded-2xl shadow-sm bg-white">
      {/* Sidebar */}
      <aside className="w-64 flex flex-col border-r border-outline-variant/30 bg-surface-container-low h-full">
